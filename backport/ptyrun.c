...
{ 
  ...
  if(name) {
    	strncpy(name, slave_name, strlen(name)-1);
		name[strlen(name)-1] = '\0';
       // printf("fd1 slave name %s",name);
  }   
  pid = fork(); 
  switch (pid) 
    { 
...
int main(int argc, char *argv[]){
    ...
    struct winsize ws;
      ...
      ws.ws_xpixel = 0;
      ws.ws_ypixel = 0;    
    char slave_name[GETPTY_BUFSIZE];
    pid=forkpty_l(&fdm, /* pseudo-terminal master end descriptor */ 
                   ...
                   &ws);/* This can be a struct winsize pointer used... */ 
    //set window size
    switch (pid) 
